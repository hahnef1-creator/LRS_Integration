...
def main():
        ...
        if gtfs.at[index,"Shape_Length"]==0:
            ...
            for point in geom.getPart(0):
                ...
                if mag == 0:
                    ...
                    the_point = arcpy.PointGeometry(point)
                    for name in column_names:
                        if name == "Shape":
                            adict["Shape"].append(the_point)
                        else:
                            adict[name].append(getattr(item,name))
                    confidence.append(1)
    ...
    storage_df = pd.DataFrame(adict,columns=column_names)
    geo.add_df_to_dbase(geodatabase,"non_confident_points",storage_df,"POINT")
